...
import { SowingAnalysisPoint } from '../types';
...
interface ChartProps {
  ...
  yAxis2Unit?: string;
  syncId?: string;
  referencePoints?: SowingAnalysisPoint[];
  horizontalReferenceLines?: { y: number; label: string; color: string }[];
  animated?: boolean;
  showGrid?: boolean;
  height?: number;
}
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="glass-dark p-4 rounded-xl shadow-2xl border border-gray-600/50 animate-scale-up">
...
const WeatherChart: React.FC<ChartProps> = ({
  title,
  data,
  type,
  xAxisDataKey,
  ...
  const renderChart = () => {
    ...
    const chartContent = (
      <>
        ...
        {horizontalReferenceLines.map((line, index) => (
          <ReferenceLine
            key={`h-ref-${index}`}
            y={line.y}
            stroke={line.color}
  ...
  return (
    <div
      className={`
        glass p-4 sm:p-6 rounded-xl shadow-lg card-hover transition-all duration-300
            ...
            <div className="flex items-center space-x-3">
              {icon && (
                <div className={`
                  ...
                  border border-primary-500/30 weather-icon
                  ${isHovered ? 'scale-110' : ''}
...
export const AnimatedLineChart: React.FC<Omit<ChartProps, 'type'>> = (props) => (
  <WeatherChart {...props} type="line" animated={true} />
);

export const AnimatedAreaChart: React.FC<Omit<ChartProps, 'type'>> = (props) => (
  <WeatherChart {...props} type="area" animated={true} />
);

export const StaticChart: React.FC<Omit<ChartProps, 'type' | 'animated'>> = (props) => (
  <WeatherChart {...props} type="line" animated={false} showGrid={false} />
);
