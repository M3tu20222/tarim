...
export async function POST(request: Request) {
  try {
    ...
    const expectedKey = process.env.MANUAL_API_KEY || process.env.CRON_API_KEY;
    ...
    return NextResponse.json({
      ok: true,
      message: "Hava durumu manuel senkronizasyonu tamamlandi",
    ...
    return NextResponse.json(
      { error: "Hava durumu manuel senkronizasyonu sirasinda hata olustu", detail: message },
      { status: 500 },
    );
  }
}